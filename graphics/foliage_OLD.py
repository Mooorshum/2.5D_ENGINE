    ...
    def update_plants(self):
        for plant in self.plants:
            for bendpoint in self.bendpoints:
                ...
                distance_to_base = sqrt((plant_x_base - bendpoint[0]) ** 2 + (plant_y_base - bendpoint[1]) ** 2)
                in_hitbox_x = (bendpoint[0] < plant_x_base + self.hitbox_lx/2) and (bendpoint[0] > plant_x_base - self.hitbox_lx/2)
                in_hitbox_y = (bendpoint[1] > plant_y_base - self.hitbox_ly/2) and (bendpoint[1] < plant_y_base + self.hitbox_ly/2)
                if in_hitbox_x and in_hitbox_y:
                    for i in range(num_leaves):
                        plant[i].external_force = self.step_bend_force * (1 - distance_to_base / self.step_effect_radius) * copysign(1, plant_x_base - bendpoint[0])
                else:
                    for i in range(num_leaves):
