...
class GrassTile:
    def __init__(self, size, position, folder, density, stiffness, cutoff_distance, relax_speed, wind_angle, num_states, scale_factor):
        self.size = size
        self.position = position
    ...
    def map_tile_images_to_rotation(self):
        ...
        for i in range(self.num_states):
            ...
            rotation = int(-self.wind_angle + 2*self.wind_angle/ (self.num_states - 1) * i)  # Compute rotation value
...
class GrassSystem:
    ...
    def apply_wind(self, omega, t):
        for tile in self.tiles:
            tilted_angle = self.wind_angle * (sin(omega * t + tile.phase_shift)) 
